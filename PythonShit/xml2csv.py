...
def xml2csv(inputfile, outputfile=None, header=False):
    ...
    ## directory with the inputfile
        ...
        with open(outputfile, 'a', encoding='utf-8') as outf:
            ## print header
            if header:
                with open(inputfiles[0], 'r', encoding='utf-8') as f:
                    soup = BeautifulSoup(f, 'html.parser')
                    taglist=[tag.name if tag.string is not None else 'None' for tag in soup.find_all()] #find("nhtsa")
                    outf.write("\",\"".join(taglist) + "\n")
            ## print cases
            for inputfile in inputfiles:
                with open(inputfile, 'r', encoding='utf-8') as f:
                    soup = BeautifulSoup(f, 'html.parser')
                    itemlist=[tag.string if tag.string is not None else 'None' for tag in soup.find_all()] #find("nhtsa")
        ...
        with open(outputfile, 'a', encoding='utf-8') as outf:
            with open(inputfile, 'r', encoding='utf-8') as f:
                ...
                if header:
                    ...
                    outf.write("\",\"".join(taglist) + "\n")
                print("SOUP IS ", soup)
                print("SOUPFIND IS ", soup) #find("nhtsa")
                itemlist=[tag.string if tag.string is not None else 'None' for tag in soup.find_all()] #find("nhtsa")
                outf.write("\"" + "\",\"".join(itemlist) + "\""+ "\n" )
        
if __name__ == "__main__":
    xml2csv((sys.argv[1].encode('utf-8').decode('utf-8')))#unicode
